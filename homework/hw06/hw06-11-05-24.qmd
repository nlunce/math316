...
eigenvector, we must find a generalized eigenvector by solving
...
This gives $v_2 = 1$. Thus, a generalized eigenvector is:
...
Straight-line solutions are of the form
$\mathbf{x}(t) = \mathbf{v} e^{\lambda t}$. Since we only have one eigenvector,
we have a solution:
...
Since we have a repeated eigenvalue and a generalized eigenvector, the general
...
$\mathbf{v}_2 = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$, we get:
...
Given $\mathbf{x}(0) = \begin{bmatrix} 1 \\ 1 \end{bmatrix}$, we apply the
initial conditions:
...
Solving these, we find $c_2 = 1$ and $c_1 = 0$. So, the solution to the
